...
def send_refund_created_email(refund_request_id: int):
    ...
    log.debug(
        "Sending refund created email to %s",
        refund_request.order.purchaser.email
    )



def send_refund_approved_email(refund_request_id: int):
    """Send refund approved email"""

    refund_request = Request.objects.get(pk=refund_request_id)

    log.debug(
        "Sending refund approved email to %s",
        refund_request.order.purchaser.email
    )
...
def send_refund_denied_email(refund_request_id: int):
    ...
    log.debug(
        "Sending refund denied email to %s",
        refund_request.order.purchaser.email
    )
